...
def get_stock_pe_at_date(stock_code, target_date):
    ...
    try:
        ...
        # 处理财务数据
        financial_data['基本每股收益'] = pd.to_numeric(financial_data['基本每股收益'], errors='coerce')
        financial_data.index = pd.to_datetime(financial_data['报告期'])
        
        # 只保留年报数据（12月31日的数据）
        annual_reports = financial_data[financial_data.index.month == 12]
...
if __name__ == "__main__":
    # 获取所有股票的PE值
    target_date = "20240301"  # 设置目标日期
    result_df = get_all_stocks_pe(target_date, max_workers=10)
